...
let GameController = {
  newGame: () => {
    return (0, _engine.default)(undefined, {
      type: "NEW_GAME"
    });
  },
  spawnBug: state => {
    return (0, _engine.default)(state, {
      type: "SPAWN_BUG"
    });
  },
  gameTick: state => {
...
let bugReducer = (state, action) => {
  ...
  switch (action.type) {
    case "SPAWN_BUG":
      return [...bugs, (0, _bug.default)(undefined, action)];

    case "GAME_TICK":
      return bugs.map(b => (0, _bug.default)(b, action, walls)).filter(b => b.gc === false);
